...
import { submitPackFunction } from '../../services/api';
import { successPackActions, errorCallingAPI } from '../../services/error'
...
class ReactWizard extends React.Component {
  ...
  submitPack = async (packID, packOperation) => {    
    try {
      const response = await submitPackFunction(packID, packOperation);               
  ...
  finishButtonClick() {    
    this.submitPack(localStorage.getItem("PackID"), localStorage.getItem("PackOperation"));
  }
...
export default withRouter(ReactWizard);
